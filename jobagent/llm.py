...
def extract_application(email: dict) -> dict:
    ...
    """
    prompt = (
        "You read a single email and decide if it concerns a real JOB the "
        "RECIPIENT applied to at an employer (application confirmation, recruiter "
        "outreach about a role they applied for, interview, rejection, or offer).\n\n"
        "Set is_job_application = FALSE for anything that is NOT an employment "
        "application, even if it says 'application' or 'apply'. In particular, "
        "exclude: online courses, bootcamps, training programs or 'learning "
        "pathways' (e.g. edtech like Coding Ninjas / careercamp), event/webinar/"
        "hackathon registrations, scholarship or membership signups, generic job "
        "alerts, newsletters, and marketing/promotions.\n\n"
        f"Sender: {email.get('sender', '')}\n"
        f"Subject: {email.get('subject', '')}\n"
        f"Body:\n{email.get('body', '')[:2000]}\n\n"
        "Extract the EXACT job title the RECIPIENT applied for "
        "(e.g. 'the Process Reengineer, NCT position' -> role 'Process Reengineer, "
        "NCT'). IGNORE roles that appear in 'reviews', 'ratings', 'salaries', "
        "'people also viewed', or other marketing/digest widgets (common in "
        "third-party notifications like AmbitionBox/Naukri) — those are NOT the "
        "applied role. Use null for role if the applied title is not clearly "
        "stated. Set status from the "
        "email's meaning: a rejection ('unfortunately', 'not progressing') -> "
        "Rejected; an interview/scheduling request -> Interview Scheduled; an "
        "offer -> Offer; a received/confirmation -> Applied.\n\n"
